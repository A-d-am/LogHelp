...
def menu_bar():
    ...
    4 - Вывести на экран сумму, потраченную на топливо 
    5 - Конец сеанса 
    """)
...
def oil_menu_bar():
    ...
    2 - Узнать сумму затрат на топливо  
...
def oil():  # часть программы для работы с топливом
    ...
    while flag:
        ...
        if choice == '1':
            my_oil.oil_cost()
            print(f'Теперь сумма затрат составляет {my_oil.get_fuel_money()}')
        elif choice == '2':
            print(my_oil.get_fuel_money())
            print()
        elif choice == '3':
            print('На данный момент сумма затрат на топливо ')
            print(f'составляет {my_oil.get_fuel_money()}')
            decision = input('Вы уверены, что хотите сохранить (1 = да, все остальное - нет) ? ')
            if decision == '1':
                my_oil.write()
                print('Данные успешно сохранены')
        elif choice == '4':
            try:
                value = float(input('На сколько хотите уменьшить сумму затрат? '))
...
def salary_menu_bar():
    print("""
    1 - Сделать список водителей (имя - зарплата за рейс) 
    2 - Удалить водителя из списка пункта 1 
    3 - Добавить водителя в список из пункта 1 
    4 - Сохранить список для дальнейшего использования
    5 - Вывести сумму всех зарплат водителей 
    6 - Выйти в главное меню
    """)
...
def salary():  # часть программы для работы с зарплатами сотрудников
    ...
    while flag:
        ...
        if choice == '1':
            ...
            my_workers.get_workers_list()  # вызывает метод, который выводит список
        elif choice == '2':
...
def main():
    ...
    while flag:
        ...
        if choice == '1':
            ...
            my_oil = OIL.Oil()
            print(f'Затраты на топливо составляют {my_oil.read_oil()}')
        elif choice == '5':
